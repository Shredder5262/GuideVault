...
function render() {
  ...
  $('gridTitle').textContent = groupMode ? pageTitleForView() : (categoryMode ? `${currentCategoryName()} Library` : 'Home Library');
...
function showDetailScreen(item) {
  ...
  document.body.classList.remove('settings-sidebar-mode', 'reader-page-mode');
  ...
  if ($('libraryView')) { $('libraryView').classList.add('hidden'); $('libraryView').style.display = 'none'; }
  if ($('settingsView')) { $('settingsView').classList.add('hidden'); $('settingsView').style.display = 'none'; }
  if ($('readerView')) { $('readerView').classList.add('hidden'); $('readerView').style.removeProperty('display'); $('readerView').style.removeProperty('visibility'); }
  if ($('detailView')) {
...
async function openReader(item) {
  ...
  clearColorscapeDetailTheme();
  document.body.classList.remove('detail-page-mode', 'settings-sidebar-mode', 'strategy-detail-mode', 'magazine-detail-mode', 'manual-detail-mode');
  ...
  if ($('settingsView')) { $('settingsView').classList.add('hidden'); $('settingsView').style.setProperty('display', 'none', 'important'); }
  if ($('detailView')) { $('detailView').classList.add('hidden'); $('detailView').style.setProperty('display', 'none', 'important'); }
  if ($('libraryView')) { $('libraryView').classList.add('hidden'); $('libraryView').style.setProperty('display', 'none', 'important'); }
  if ($('readerView')) {
    $('readerView').classList.remove('hidden');
    $('readerView').style.setProperty('display', 'flex', 'important');
    $('readerView').style.setProperty('visibility', 'visible', 'important');
  }
...
function showLibraryScreen() {
  ...
  if ($('libraryView')) { $('libraryView').classList.remove('hidden'); $('libraryView').style.display = ''; }
}
function showSettingsScreen(tab = 'account') {
  cleanupInactiveViewsForNavigation('settings');
  clearColorscapeDetailTheme();
  document.body.classList.remove('detail-page-mode', 'reader-page-mode');
  document.body.classList.add('settings-sidebar-mode');
  if ($('readerView')) { $('readerView').classList.add('hidden'); $('readerView').style.removeProperty('display'); $('readerView').style.removeProperty('visibility'); }
  if ($('detailView')) $('detailView').classList.add('hidden');
  if ($('libraryView')) { $('libraryView').classList.add('hidden'); $('libraryView').style.display = 'none'; }
  if ($('settingsView')) { $('settingsView').classList.remove('hidden'); $('settingsView').style.display = ''; }
...
function activateSettingsTab(tab = 'account') {
  ...
  if ($('settingsImportTitle')) $('settingsImportTitle').textContent = libraryMode ? 'Library' : 'Import';
  if ($('settingsImportSub')) $('settingsImportSub').textContent = libraryMode
    ? 'Manage stored library paths, rescan individual folders, and edit scan-in-place library entries.'
    : 'Build multiple libraries by scanning existing folders in place. Source files stay where they already are.';
}
